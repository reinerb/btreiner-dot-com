...
export default async function handler(
  ...
  const { formValues, token } = JSON.parse(req.body);
  ...
  try {
    ...
    // Convert formValues into a FormData
    const keys = Object.keys(formValues) as string[];
    const submission = new FormData();

    keys.forEach((key) => submission.append(key, formValues[key]));
    submission.append("subject", "New message from btreiner.com contact form");
    submission.append("access_key", WEB3FORMS_KEY);
